...
class NotificationSystemTestCase(TestCase):
    ...
    def test_like_notification(self):
        ...
        response = self.client.post(reverse('like-post', kwargs={'post_id': self.post.id}))
        self.assertEqual(response.status_code, 201)
        notif = Notification.objects.filter(recipient=self.user1, notification_type='like').first()
        self.assertIsNotNone(notif)
        self.assertIn('liked your post', notif.message)

    def test_reply_notification(self):
        comment = Comment.objects.create(user=self.user1, post=self.post, content='Nice post!')
        self.client.force_authenticate(user=self.user2)
        url = reverse('comment-reply', kwargs={'post_id': self.post.id, 'parent_id': comment.id})
        response = self.client.post(url, {'content': 'Thanks!'}, format='json')
        self.assertEqual(response.status_code, 201)
        notif = Notification.objects.filter(recipient=self.user1, notification_type='reply').first()
        self.assertIsNotNone(notif)
