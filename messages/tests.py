...
class MessageNotificationTests(APITestCase):
    def setUp(self):
        ...
        self.recipient = CustomUser.objects.create_user(
            username="recipient", password="pass1234"
        )
    ...
    def test_send_message_and_notification(self):
        ...
        msg = Message.objects.filter(
            sender=self.sender, recipient=self.recipient, content="Hello there!"
        ).first()
    ...
    def test_send_image_message_and_notification(self):
        ...
        self.assertEqual(response.status_code, 201)
        msg = Message.objects.filter(
            sender=self.sender, recipient=self.recipient, image__isnull=False
        ).first()
        self.assertIsNotNone(msg)
    ...
    def test_send_video_message_and_notification(self):
        ...
        self.assertEqual(response.status_code, 201)
        msg = Message.objects.filter(
            sender=self.sender, recipient=self.recipient, video__isnull=False
        ).first()
        self.assertIsNotNone(msg)
...
class TestWebSocketRealtime:
    async def test_realtime_notification(self, db):
        ...
        communicator = WebsocketCommunicator(
            application, f"/ws/notifications/{user.id}/"
        )
    ...
    async def test_realtime_user_message(self, db):
        ...
        sender = CustomUser.objects.create_user(
            username="wssender", password="pass1234"
        )
        recipient = CustomUser.objects.create_user(
            username="wsrecipient", password="pass1234"
        )
...
class TestWebSocketRealtimeMessage:
    async def test_realtime_message_content_and_media(self, db):
        ...
        sender = CustomUser.objects.create_user(
            username="ws_sender", password="pass1234"
        )
        recipient = CustomUser.objects.create_user(
            username="ws_recipient", password="pass1234"
        )
        # Connect as recipient to their chat group
        communicator = WebsocketCommunicator(
            application, f"/ws/chat/{sender.id}/{recipient.id}/"
        )
